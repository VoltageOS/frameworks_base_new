...
class CaffeineDialogDelegate @Inject constructor(
...
) : SystemUIDialog.Delegate, CaffeineController.CaffeineStateListener {
    ...
    private val iconCaffeine: Drawable get() = ContextCompat.getDrawable(ctx, R.drawable.ic_qs_caffeine)!!
    ...
    override fun beforeCreate(d: SystemUIDialog, b: Bundle?) {
        d.setTitle(R.string.qs_caffeine_dialog_title)
        d.setView(layoutInflater.inflate(R.layout.qs_caffeine_dialog, null))
    ...
    override fun onCreate(d: SystemUIDialog, savedInstanceState: Bundle?) {
        statusText = d.requireViewById(R.id.caffeine_status_text)
        doneButton = d.requireViewById(com.android.internal.R.id.button1)
        slider = d.requireViewById(R.id.caffeine_slider)
    ...
    private fun setupSlider() {
        ...
        slider.trackIconActiveStart = iconCaffeine
    ...
    private fun getLabelForIndex(index: Int): String {
        ...
        return when (mins) {
            0 -> ctx.getString(R.string.accessibility_quick_settings_caffeine_off)
    ...
    override fun onCaffeineStateChanged(active: Boolean, label: String) {
        if (!userTracking) {
            ...
            val mins = controller.durations[currentIndex]
            if (active && mins > 0) {
                statusText.text = label
            } else {
                updateStatusText(currentIndex)
            }
        }
    }
}
