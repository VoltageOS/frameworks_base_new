...
public class GlobalActionsDialogLite implements DialogInterface.OnDismissListener,
    ...
    @Nullable
    private ContentObserver mPowerMenuActionsObserver;
    @Nullable
    ...
    public GlobalActionsDialogLite(
        ...
        if (mLineageGlobalActions != null) {
            mPowerMenuActionsObserver = new ContentObserver(mMainHandler) {
                @Override
    ...
    public void destroy() {
        ...
        if (mPowerMenuActionsObserver != null) {
    ...
    private List<GlobalActionType> getConfiguredActionTypes() {
        if (mActions == null) {
            return mInteractor.getPossibleGlobalActions();
        }

        List<GlobalActionType> actionTypes = new ArrayList<>();
        for (String actionKey : mActions) {
            GlobalActionType actionType = GlobalActionType.fromConfigKey(actionKey);
            if (actionType == null) {
                Log.w(TAG, "Unknown configured global action: " + actionKey);
            } else if (!actionTypes.contains(actionType)) {
    ...
    void createActionItems() {
        ...
        List<GlobalActionType> actionTypes = getConfiguredActionTypes();

        ShutDownAction shutdownAction = new ShutDownAction();
        RestartAction restartAction = new RestartAction();
        RestartSystemAction systemRestartAction = new RestartSystemAction();
        RestartRecoveryAction recoveryRestartAction = new RestartRecoveryAction();
        RestartBootloaderAction bootloaderRestartAction = new RestartBootloaderAction();
        RestartDownloadAction downloadRestartAction = new RestartDownloadAction();
        RestartFastbootAction fastbootRestartAction = new RestartFastbootAction();
        RestartSystemUIAction sysuiRestartAction = new RestartSystemUIAction();
        ...
        for (String actionKey : getRestartActions()) {
            if (RESTART_ACTION_KEY_RESTART.equals(actionKey)) {
                addIfShouldShowAction(mRestartItems, systemRestartAction);
            } else if (RESTART_ACTION_KEY_RESTART_RECOVERY.equals(actionKey)) {
                addIfShouldShowAction(mRestartItems, recoveryRestartAction);
            } else if (RESTART_ACTION_KEY_RESTART_BOOTLOADER.equals(actionKey)) {
                addIfShouldShowAction(mRestartItems, bootloaderRestartAction);
            } else if (RESTART_ACTION_KEY_RESTART_DOWNLOAD.equals(actionKey)) {
                addIfShouldShowAction(mRestartItems, downloadRestartAction);
            } else if (RESTART_ACTION_KEY_RESTART_FASTBOOT.equals(actionKey)) {
                addIfShouldShowAction(mRestartItems, fastbootRestartAction);
            }
        }

        if (mLineageGlobalActions != null &&
                mLineageGlobalActions.userConfigContains(RESTART_ACTION_KEY_RESTART_SYSTEMUI)) {
            addIfShouldShowAction(mRestartItems, sysuiRestartAction);
        }
    ...
    final class RestartAction extends SinglePressAction implements LongPressAction {
        RestartAction() {
            super(com.android.systemui.res.R.drawable.ic_global_actions_restart, shouldShowRestartSubmenu()
    ...
    private void addUserActions(List<Action> actions, UserInfo currentUser) {
        ...
        for (final UserInfo user : mUserManager.getUsers()) {
            if (!user.isUiSwitchableHumanUser()) {
                continue;
            }
            boolean isCurrentUser = currentUser == null
                    ? user.id == UserHandle.USER_SYSTEM : currentUser.id == user.id;
            ...
            Drawable avatar = new BitmapDrawable(mContext.getResources(),
    ...
    public class MyAdapter extends MultiListAdapter {
        ...
        public void onClickItem(int position) {
            ...
            if (!(item instanceof SilentModeTriStateAction)) {
                if (mDelegate != null) {
                    // Don't dismiss while opening a submenu.
    ...
    private static Bitmap createCircularClip(Bitmap input, int width, int height) {
        ...
        canvas.drawCircle(input.getWidth() / 2f, input.getHeight() / 2f,
                Math.min(input.getWidth(), input.getHeight()) / 2f, paint);
        return output;
    }
    @NonNull
    @Override
